...
const EditCountry: React.FC = () => {
    ...
    const fetchCountry = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          utcHoursDifference: data.utcHoursDifference?.toString() || ''
        });
        setLoading(false);
      } catch (err) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await apiClient.putApiMasterV1Countries(dto);
      navigate(`/country/view/${dto.id}`);
    } catch (err) {
  ...
  const handleCancel = () => {
    if (country?.id) {
      navigate(`/country/view/${country.id}`);
    } else {
      navigate('/countries');
    }
  };
  ...
  if (error && !country) {
    return (
      <div className="user-profile-page">
        ...
        <button className="back-button" onClick={() => navigate('/countries')} style={{ marginTop: '20px' }}>
          ← {t('Button_Back')}
        </button>
      </div>
    );
      ...
      <div className="profile-card">
        ...
        <form onSubmit={handleSubmit} className="profile-form">
          ...
          <div className="form-section">
            <h3 className="section-title">{t('EditCountryPage_BasicInfo')}</h3>

...
const CreateLanguage: React.FC = () => {
	...
	const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			const created = await apiClient.postApiMasterV1Languages(dto);
			navigate(`/language/view/${created.id}`);
		} catch (err) {
			...
			<div className="profile-card">
				<div className="profile-avatar-section">
					...
					<h2 className="profile-name">
						{formData.nameTranslationKey || formData.id || t('NewLanguagePage_NewLanguage')}
					</h2>
				</div>
				...
				<form onSubmit={handleSubmit} className="profile-form">
					...
					<div className="form-section">
						<h3 className="section-title">{t('NewLanguagePage_BasicInfo')}</h3>
